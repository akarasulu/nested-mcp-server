...
- Storage volume upload/download
- NUMA topology controls
...
| Domain | vCPU and memory tuning | Implemented | set_domain_vcpus, set_domain_memory | Add maximum vCPU control, NUMA topology | P2 |
| Domain | Domain statistics | Implemented | get_domain_stats, get_domain_block_stats, get_domain_interface_stats, get_domain_memory_stats | Add event feeds and NUMA stat surfaces | P2 |
| Domain | CPU pinning | Implemented | get_domain_vcpu_pin_info, set_domain_vcpu_pin, get_domain_emulator_pin_info, set_domain_emulator_pin | Add NUMA topology and vCPU placement controls | P2 |
...
| Storage | Volume inspect and lifecycle | Implemented | list_storage_volumes, get_storage_volume, create_storage_volume_xml, delete_storage_volume, clone_storage_volume, resize_storage_volume, wipe_storage_volume | Add upload/download and metadata operations | P1 |
...
| CPU and memory runtime controls | Implemented | qmp_balloon, qmp_query_hotpluggable_cpus, qmp_cpu_add, qmp_query_memory_devices, qmp_object_add, qmp_object_del | NUMA topology placement not yet |
...
### Phase B: Advanced operator parity (largely complete)
...
- Remaining: storage volume upload/download, NUMA topology controls.

### Phase C: Specialized parity (deferred)
...
1. Storage parity: volume resize and wipe operations.
2. Block job parity: block mirror, backup, and progress polling.
3. QMP vCPU hotplug and memory device controls.
4. Domain checkpoint NBD export integration.
5. QMP migration telemetry (query-migrate, query-migrate-capabilities).
